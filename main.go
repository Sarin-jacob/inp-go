...
// FaceKey represents a sorted sequence of node IDs used to uniquely hash a face.
// The 4th element is 0 for triangles.
type FaceKey [4]int
	...
	for scanner.Scan() {
		...
		if state == "NODE" && len(parts) >= 3 {
			...
			for i, p := range parts {
				...
				if err == nil {
					// Skip the element ID (first part of a new line if not a continuation)
					if i == 0 && len(currentNodes) == 0 {
						continue
					}
...
// --- MANIFOLD EXTRACTION (CONCURRENCY) ---
...
func extractFaces(etype string, enodes []int) [][]int {
	...
	if strings.Contains(upType, "3D8") || strings.Contains(upType, "3D20") {
		if len(enodes) >= 8 {
			n := enodes
			return [][]int{
				{n[0], n[1], n[2], n[3]}, {n[4], n[7], n[6], n[5]},
...
func processChunk(elements []Element, resultChan chan<- map[FaceKey][]int, wg *sync.WaitGroup) {
	...
	// Filter down to local exterior faces before sending over channel
	localShell := make(map[FaceKey][]int)
	for k, count := range counts {
		if count == 1 {
			localShell[k] = faceMap[k]
		}
	}
...
func exportOBJ(path string, nodes map[int]Node, outerFaces [][]int) {
	...
	// Map original INP node IDs to sequential 1-based OBJ indices
	...
	for nid := range usedNodes {
		orderedIDs = append(orderedIDs, nid)
	}
	...
	for _, f := range outerFaces {
		...
		for _, nid := range f {
			writer.WriteString(fmt.Sprintf(" %d", nodeToObjIdx[nid]))
		}
		writer.WriteString("\n")
	}
	writer.Flush()
...
func exportGLB(path string, nodes map[int]Node, outerFaces [][]int) {
	...
	// Populate Geometry and mock FEA vertex coloring (Z-height heatmap)
	for i, nid := range orderedIDs {
		nodeToIdx[nid] = uint32(i)
		n := nodes[nid]
		vertices = append(vertices, n.X, n.Y, n.Z)

		// Heatmap simulation based on Z height (Mapped to 0.0 - 1.0 for GLB)
		var val float32 = n.Z
		if val < 0 {
			val = -val
		}
	...
	// Pad buffer to 4 bytes boundary for indices
	for binBuffer.Len()%4 != 0 {
		binBuffer.WriteByte(0)
	}
	...
	jsonStr := fmt.Sprintf(`{
		...
		"materials": [{"pbrMetallicRoughness": {"baseColorFactor": [1, 1, 1, 1], "metallicFactor": 0.1, "roughnessFactor": 0.5}, "doubleSided": true}],
		...
		"accessors": [
			...
			{"bufferView": 1, "byteOffset": 0, "componentType": 5126, "count": %d, "type": "VEC4"},
			{"bufferView": 2, "byteOffset": 0, "componentType": 5125, "count": %d, "type": "SCALAR"}
		]
	}`,
		binBuffer.Len(),
		len(vertices)*4,                // Positions
		len(vertices)*4, len(colors)*4, // Colors
		indicesOffset, len(indices)*4, // Indices
		len(vertices)/3, len(colors)/4, len(indices),
	)
	...
	// 3. Write GLB File (Header + JSON Chunk + BIN Chunk)
...
func main() {
	formatPtr := flag.String("format", "both", "Output format: obj, glb, both")
	flag.Parse()

	if len(flag.Args()) < 1 {
		fmt.Println("Usage: inpparser -format=both <file.inp>")
		return
	}
	inpPath := flag.Args()[0]
